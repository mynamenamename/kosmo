...
class A {    //클래스: 객체를 생성하기 위한 설계도
	// class 내부에 올 수 있는 것 4가지 : 1. 필드(클래스에서 선언된 변수), 2. 생성자 , 3. 메소드 , 4. 이너클래스
	//1. 필드
	...
	//       기본생성자(입력매개변수가 없는 생성자)는 생략될 수 있다. A () {}
	//       객체 생성시 생성자 호출
	//       객체의 초기값을 셋팅
	//       기본 생성자는 생략되어 있을 경우 컴파일러가 자동으로 등록해준다.
	//       클래스 내부의 다른 생성자가 존재할 때, 컴파일러가 기본 생성자를 자동으로 만들어주지 않는다.
...
public class Using_Object {
	...
	public static void main(String[] args) {
	    ...
	    //메소드 호출(work2(정수,정수))
	    int k =a1.work2(10, 20);         //매개변수가 2개인 메소드 << return a+b;
	    ...
	    //메소드 호출(work3(정수,실수,문자열)) - 리턴 double
	    ...
	    String s = a1.work4("안녕");        //리턴받은 값을 변수에 할당 후 출력 
	    System.out.println(s);
	    System.out.println("==================");
	    System.out.println(a1.work4("하세요"));   //인풋값을 리턴받음
	    
	    
	    // 클래스 : 객체를 생성하기 위한 설계도  <클래스 영역에 로드됨>
	    // 객체 : 클래스를 기반으로 만들어진 인스턴스(객체) <Heap에 로드됨>
	    // 인스턴스화 : 클래스는 객체화 해야 사용가능 A a = new a();
